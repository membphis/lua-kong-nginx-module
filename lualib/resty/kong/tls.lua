...
if ngx.config.subsystem == "http" then
    ...
    int ngx_http_lua_kong_ffi_set_upstream_trusted_store(ngx_http_request_t *r,
        void *_store);
...
if ngx.config.subsystem == "http" then
    ...
    do
        ...
        function _M.set_upstream_trusted_store(store)
            if not ALLOWED_PHASES[get_phase()] then
                error("API disabled in the current context", 2)
            end

            if not store then
                error("store must not be nil", 2)
            end

            local r = get_request()

            local ret = C.ngx_http_lua_kong_ffi_set_upstream_trusted_store(
                r, store)
            if ret == NGX_OK then
                return true
            end
        ...
        function _M.set_upstream_ssl_verify_depth(depth)
            ...
            if not tonumber(depth) then
                error("depth must be a number", 2)
            end
